...
class Category(models.Model):
    ...
    subscribers = models.ManyToManyField(User, blank=True, related_name='subscriber')
...
class Post(models.Model):
    ...
    def __str__(self):
        return f"{self.title}, дата добавления: {self.post_data.strftime('%Y-%m-%d %H:%M:%S')}" # в админке отображается название заголовка, а не Post.objects
...
class PostCategory(models.Model):
    ...
    def __str__(self):
        return f'{self.postc_link_post} | {self.postc_link_category}'
